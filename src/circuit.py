...
def QCNN_structure(params, unitary_func, params_per_unitary):
    ...
    for i in range(0, 8, 2):  # Even pairs
        unitary_func(params[param_index:param_index+params_per_unitary], [i, i+1])
        param_index += params_per_unitary
    
    for i in range(1, 7, 2):  # Odd pairs
        unitary_func(params[param_index:param_index+params_per_unitary], [i, i+1])
        param_index += params_per_unitary
    
    unitary_func(params[param_index:param_index+params_per_unitary], [7, 0])  # Circular
    param_index += params_per_unitary
    ...
    for i in range(4, 8, 2):  # Even pairs
        unitary_func(params[param_index:param_index+params_per_unitary], [i, i+1])
        param_index += params_per_unitary
    
    unitary_func(params[param_index:param_index+params_per_unitary], [7, 4])  # Circular
    param_index += params_per_unitary
